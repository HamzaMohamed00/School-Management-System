...
export class GradeService {
  ...
  async getTeacherSetup(): Promise<any> {
    return this.api.get('/api/Grade/teacher-setup');
  }

  async getClassGrades(classId: number, subjectName: string, gradeType: string): Promise<any[]> {
    return this.api.get<any[]>(`/api/Grade/class-grades?classId=${classId}&subjectName=${encodeURIComponent(subjectName)}&gradeType=${encodeURIComponent(gradeType)}`);
  }

  async bulkUpdateGrades(payload: any): Promise<any> {
    return this.api.post('/api/Grade/bulk-update', payload);
  }
}