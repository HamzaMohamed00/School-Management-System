        ...
        <div class="col-12 col-md-8 col-lg-6">
          <div class="card border-0 rounded-4 shadow-sm">
            <div class="card-header bg-transparent border-bottom p-4 d-flex align-items-center gap-3">
              <button class="btn btn-icon btn-light rounded-circle" [routerLink]="isTeacher ? '/admin/teachers' : '/admin/students'">
                <i class="fas fa-arrow-right"></i>
              </button>
              <div>
                <h5 class="fw-bold mb-1">تدريب بصمة الوجه</h5>
                <small class="text-muted" *ngIf="user">{{ isTeacher ? 'للمدرس' : 'للطالب' }}: {{ user.fullName }}</small>
              </div>
            </div>
...
export class TrainFaceComponent implements OnInit {
    ...
    async ngOnInit() {
        ...
        this.isTeacher = this.router.url.includes('/teachers/');

        if (!this.id) {
            this.router.navigate([this.isTeacher ? '/admin/teachers' : '/admin/students']);
            return;
        }
        await this.loadData();
    }
    ...
    async loadData() {
        try {
            if (this.isTeacher) {
                this.user = await this.teacherService.getTeacherById(this.id);
            } else {
                this.user = await this.studentService.getStudentById(this.id);
            }
        } catch {
            this.notification.error(this.isTeacher ? 'فشل في تحميل بيانات المدرس' : 'فشل في تحميل بيانات الطالب');
            this.router.navigate([this.isTeacher ? '/admin/teachers' : '/admin/students']);
        }
    }
    ...
    async captureAndTrain() {
        ...
        try {
            ...
            const file = new File([blob], `train_${this.isTeacher ? 'T' : 'S'}_${this.id}.jpg`, { type: 'image/jpeg' });

            if (this.isTeacher) {
                await this.teacherService.trainFace(this.id, file);
            } else {
                await this.studentService.trainFace(this.id, file);
            }
