...
public class TokenService : ITokenService
{
    ...
    public string CreateToken(string userId, string email, string role, string fullName, string avatarUrl = null, int? entityId = null)
    {
        ...
        if (entityId != null)
        {
            claims.Add(new Claim("EntityId", entityId.ToString()));
        }

        if (!string.IsNullOrEmpty(avatarUrl) && avatarUrl.Length < 1000) 
        {
